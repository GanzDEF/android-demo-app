...
class WeatherInfoInteractorTest {
    ...
    val weatherRepository = mock<WeatherRepository> {
        val successObservable = Observable.just(Result.createWeatherResultSuccessResponse(10.0))
        val requiredCityErrorObservable = Observable.error<Result>(Exception("Invalid city provided"))

        on {getWeatherInfo(eq(CITY), anyString(), anyString())}         doReturn successObservable
        on {getWeatherInfo(eq(INVALID_CITY), anyString(), anyString())} doReturn requiredCityErrorObservable
    }
    ...
    fun setup() {
        init()
        testSubject = WeatherInteractorImpl(greetRepository, weatherRepository)
    }
